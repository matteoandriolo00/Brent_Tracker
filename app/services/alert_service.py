...
def check_and_trigger_alerts(db: Session, current_price: float):
    ...
    """
    # Gestione alert "BELOW": scattano se il prezzo corrente è MINORE o UGUALE alla soglia
    ...
    # Gestione alert "ABOVE": scattano se il prezzo corrente è MAGGIORE o UGUALE alla soglia
    db.query(Alert).filter(
        Alert.status == "ACTIVE",
    ...
    # Salva i cambi di stato sul database
    db.commit()